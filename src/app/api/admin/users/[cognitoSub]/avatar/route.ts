...
export async function GET(_request: Request, { params }: { params: Promise<{ cognitoSub: string }> }) {
  ...
  const response = await fetch(`${serverEnv.BACKEND_API_URL}/users/${cognitoSub}/avatar/download-url`, { headers: { Accept: "application/json", Authorization: `Bearer ${token ?? ""}` }, cache: "no-store" });
